...
{
	struct bitset * set_a = bitset_new(size);
	struct bloom * filter_a = malloc(sizeof(struct bloom));
	filter_a->bit_vector=set_a;
	filter_a->size=size;
	return filter_a;
}
...
int bloom_lookup(struct bloom * this, char * item)
{
	...
	int isHash1 = bitset_lookup(this->bit_vector, hash1);
	int isHash2 = bitset_lookup(this->bit_vector, hash2);
	return (isHash1 && isHash2);;		
}
